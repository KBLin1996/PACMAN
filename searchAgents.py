...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        successors = []         # Initialized successors
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #   hitsWall = self.walls[nextx][nexty]
              ...
              # cornersVisited records which corners had been visited
            cornersVisited = state[1]
            x, y = state[0]    # Take the state position as x, y
            dx, dy = Actions.directionToVector(action)  # The vector of the next step
              # The next step = (nextx, nexty)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]

            if not hitsWall:    # If (nextx, nexty) aren't walls
                nextState = (nextx, nexty)  # Store (nextx, nexty) in nextState
                if nextState in self.corners:
                    if nextState not in cornersVisited:
                        cornersVisited = (nextState, ) + cornersVisited
                  # Store the cost to nextState in cost
                cost = self.costFn(nextState)
                  # Successors is a tuple for returning
                successors.append(((nextState, cornersVisited), action, cost))

          # Bookkeeping for display purposes
        self._expanded += 1     # Number of search nodes expanded + 1
        return successors       # Return the successors
...
def cornersHeuristic(state, problem):
    ...
    corners = problem.corners   # These are the corners' coordinates

      # Calculate the distance from current position to the corner
    def Manhattan(pos, goal):   # Manhattan(state[0], corner)
        return abs(pos[0] - goal[0]) + abs(pos[1] - goal[1])

      # Store the corners which the successor hasn't visited
    unCornerVisited = [corner for corner in corners if corner not in state[1]]
    cost = 0    # Initialized cost to 0

      # We have to check which combination of corners is the best solution
    for unVisited in unCornerVisited:
          # Accumulate the cost of the combination of corners
        cost += Manhattan(state[0], unVisited)
